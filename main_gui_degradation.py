...
class main_gui:
    ...
    machine = EnigmaMachine.from_key_sheet(
       rotors='II IV V',
       reflector='B',
       ring_settings=[21, 11, 25],
       plugboard_settings='AV BS CG DL FU HZ IN KM OW RX')

    lowCVpotential = 0 # lowest point in voltage in your CV
    highCVpotential = 0 # highest -''-
    rateOfCV = 0 # mV/s
    nDegradCycles = 0 # how many degradation cycles (CVs) to run
    scansPerCycle = 0 # how many scans per degrad cycle, set it the same as in ESR studio!

    def __init__(self): # easy, EASY!
    
        # a little bit of encryption
        self.machine.set_display('EAC')
        msg_key = self.machine.process_text('RST')

        from random import randrange
        
        file1 = open('haram.txt','r')  # create and open a file for writing
        lines = file1.readlines()
        file1.close()
        cprtxt = lines[randrange(len(lines))]
        self.machine.set_display('EAC')
        msg_key = self.machine.process_text('RST')
        qut = self.machine.process_text(cprtxt)
        input(qut[:-1])
        
        # --------------------------
        self.lowCVPotential = float(input('LOW Potential (vs. your RE), V ? ')) # reduced potential input
        self.highCVpotential = float(input('HIGH Potential (vs. your RE), V ?')) # oxidised potetnial input
        self.rateOfCV = float(input('rate of CV, mV/s ? ')) # number of steps between red and ox potentials
        self.nDegradCycles = int(input('degradation cycles ?')) # how many degrad cycles
        self.scansPerCycle = int(input('How many accumulations in [ESR Studio] ?')) # how many accum
        while (self.scansPerCycle < 2):
            print('Hey, minimum 2 scans per CV. or fix it yourself!')
            self.scansPerCycle = int(input('How many accumulations in [ESR Studio] ?')) # how many accum
        ...
        print('navigate to the folder that you created in ESR studio, don''t forget to switch back to the Recipe Editor tab!')
...
#!!! changed here nscan label name
     ...
     #   self.TEntry1.configure(cursor="ibeam")
        ...
        self.cvPlotter = Plotter.CvPlotter(rootframe=self.cvPlotter_frame,rootWindow = self.window)
        ...
        self.run_btn.configure(command = self.runDegradationExperiment)  # !!!!! here is the duration of the transient!
...
########################################################################################################################
########################################################################################################################
# -------------------- MAGNETTECH ---------------- D E G R A D A T I O N -----------------------------------------------
########################################################################################################################
########################################################################################################################
########################################################################################################################
########################################################################################################################
    ...
    def runDegradationExperiment(self):
        ...
        for degradCycleNumber in range (self.nDegradCycles):
            print('============== DEGRADATION CYCLE %d ==============='%degradCycleNumber)
            
            # get cv
            print('getting CV. degradation cycle # %d'%degradCycleNumber)
            cvFileNameString = '%s_degradation_cycle_%d'%(self.filePath,degradCycleNumber)  
            TakeCv(self.spectrometer_communicator, self.lowCVpotential, self.highCVpotential, self.rateOfCV,cvFileNameString)
            # get cwepr spectra
            print('getting spectra # %d'%degradCycleNumber)
            fileNameString = self.filePath+'_cycle_%d'%degradCycleNumber+'_'+str(int(self.scansPerCycle))+'_scans'
            cw_scan_autohotkey(fileNameString,os.path.dirname(self.filePath),int(self.scansPerCycle)) # autohotkey triggers the magnettech software here. Just run the script. And save the time stamp!


        print('keithley sets the potential to 0 V and shuts the output down')
        self.spectrometer_communicator.pstat.output_off()  # включили выход потенциостата, он - NE ебашит.
 
        if self.nDegradCycles < 5:
            print('only %s cycles? You must be testing it. Do more!'%self.nDegradCycles)
        else:
            RunAsurvey()
    ...
    def runPotentialSweepExperiment(self):
        ...
        f2w = open('%s_EPR_FILENAMES.txt'%filePath,'w')  # create and open a file for writing
...
##################################################################################################################################
    ...
    def connect_to_spectrometer(self):
        ...
        #MESS!
        print('you are good. keep pressing buttons. Set up scan -> Set CW Scan')
...
def RunAsurvey():
    ...
    print('setting the potential to 0, output off')
    print('\n___________________-> fireworks ***************************____________________')
    print('\n_______________________________________________________________________________')
    print('============== CONGRATULATIONS, YOU HAVE SUCCESSFULLY DEGRADED. ===============')
    print('__________________*****************************************____________________\n')
    print('Choose your level of degradation:') 
    
    listOfCreatures = ['Joshka','Domenik','Homo Sapiens','Homo Neanderthalensis','Homo Erectus',
                       'Last common ancestor shared by humans, gorillas and bonobos','Flat worm',
    ...
    if 'Marcel' in listOfCreatures[degradationCode]:
        input('Please enter your bank account password:')
...
def TakeCv(sp_com: communication.new_communicator, lowPotential:float, highPotential:float,rate:float,filePath:str):
    print('take a cv curve and save it in %s.csv'%filePath)
    sp_com.pstat.play_reading_beep()
    sp_com.pstat.TakeCV(lowPotential,highPotential,rate,filePath)
