...
class main_gui:
    ...
    def __init__(self): # easy, EASY!
        ...
        self.scansPerStep = int(input('scans per step for RT [ESR studio!] ?')) # how many scans at RT ESR studio takes per potential
...
########################################################################################################################
    ...
    def runPotentialSweepExperiment(self):
        ...
        # check if there are already dummy_files in the directory of the potential series
        dummypattern = '%s/Potential_Series/*dummy.xml'%(os.path.dirname(self.filePath))
        print(dummypattern)
        dummyfiles = glob.glob(dummypattern)
        ...
        for i in range(self.nSteps*2+1):
            ...
            cw_scan_sequence_autohotkey(self,fileNameString,os.path.dirname(self.filePath),int(self.scansPerStep),i+len(dummyfiles)) # autohotkey triggers the magnettech software here. Just run the script. And save the time stamp!
...
# this is the adjusted function to start the automated cooling cycle sequence in Magnettech
# AUTOEMRESequence.ahk from the desktop has to be running to work
# In the set folder in ESRStudio create the folders "RT_spectra" and "Cooled_spectra"

def cw_scan_sequence_autohotkey(self,filepath, datapath, nscans,i): # nscans is a parameter. set the same in magnettech for RT and cooled spectra! i is given to recognise the number of dummyfiles in the folder
    # time paradoxes if seconds are more than 55, 
    nowmoment = datetime.now()
    if nowmoment.second >= 55:
        print('\n - - - time paradox detected - - -\n')
    ...
    # RT scans, might not work if the temperature is too unstable, since the stabilization takes too long
    # check with glob.glob how many files are there. if files as nscans: 
    numberoftrials = 0 # trials to find all spectra in the current folder
    maxnumberoftrials = 1200 # you got 20 minutes to get the first scan in the folder
    while numberoftrials <= maxnumberoftrials:
        numberoftrials = numberoftrials + 1
        sleep(1) # check floder each 5 seconds
        print('searching for RT spectra, attempt %d / %d \n path:'%(numberoftrials,maxnumberoftrials))
        patternString = '%s/Potential_Series/RT_spectra/%s/*.xml'%(datapath,startTimeStamp)
        print(patternString)
        genFiles = glob.glob(patternString)
        print(genFiles)
        print(len(genFiles),' of ',nscans,' spectra recorded')
    ...
    # averaged scan:
    avscanFile = glob.glob('%s/Potential_Series/RT_spectra/*%s.xml'%(datapath,startTimeStamp)) # find the file via the second timestamp
    # good for plotting the EPR data:
    ACCUMULATEDSPECTRAFILENAMES.append(avscanFile) # append the paths of the complete scans to the list of names 
    #write spectral file names to file
    ...
    # detect dummy file at the end of the sequence, time to finish cooling, spectrum accumulation and heating back to RT should be shorter than 3h 20min
    numberoftrials = 0
    while numberoftrials <= maxnumberoftrials:
        numberoftrials = numberoftrials + 1
        sleep(10) # check folder each 10 seconds
        print('searching for dummy file, attempt %d / %d \n path:\n'%(numberoftrials,maxnumberoftrials))
        dummypattern = '%s/Potential_Series/*dummy.xml'%(datapath)
        print(dummypattern)
        dfiles = glob.glob(dummypattern)
        if (len(dfiles) == i+1): # if there are as many dummyfiles as steps performed, continue with next step
            print('Cooling cycle %d finished. Moving to the next potential'%(i))    
